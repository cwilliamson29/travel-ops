...
const Marquee = () => {
  ...
  const renderMarqueeElements = () => {
    ...
    return marqueeTexts.map((e, i) => (
      <p className=" text-center px-4 text-2xl font-semibold absolute pin-l w-1/2" key={`marquee-${i}`} ref={(el) => marqueeElementsRefHandler(el, i)}>
        {e}
      </p>
    ));
  };
  return (
    <div className=" relative w-screen mt-8 py-4 bg-green-600 text-gray-200 flex overflow-hidden items-center" style={{ minHeight: '110px' }}>
